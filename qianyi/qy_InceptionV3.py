...
def loadfile():
    ...
    labels = []
    # 拿到图像数据路径，方便后续读取
    imagePaths = sorted(list(list_images('./dataset/set1')))
    random.seed(42)
...
## 建立卷积神经网络
    model = SimpleVGGNet.build(width=75, height=75, depth=3,classes=len(lb.classes_))
#    # 设置初始化超参数
    INIT_LR = 0.01
    EPOCHS = 50
    BS = 32
...
#    # 绘制结果曲线
    ...
    plt.legend()
    plt.savefig('./output/inception_set1.png')
    #
    # 保存模型
    print("------正在保存模型------")
    model.save('./output/inception-set1.model')
    f = open('./output/inception.pickle', "wb")
