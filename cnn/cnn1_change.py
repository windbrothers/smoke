...
def loadfile():
    ...
    labels = []
    # 拿到图像数据路径，方便后续读取
    imagePaths = sorted(list(list_images('./dataset/set7')))
    random.seed(42)
...
#    # 绘制结果曲线
    ...
    plt.legend()
    plt.savefig('./output/cnn1_change.png')
    #
    # 保存模型
    print("------正在保存模型------")
    model.save('./output/cnn1_change.model')
    f = open('./output/cnn1_change.pickle', "wb")
